...
export const api = {
  ...
  players: {
    ...
    getByPosition: (position, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.players?.getByPosition?.(position) || apiClient.get(`/api/players?position=${position}`)
        : apiClient.get(`/api/players?position=${position}`),
    getByTeam: (teamId, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.players?.getByTeam?.(teamId) || apiClient.get(`/api/players?team=${teamId}`)
        : apiClient.get(`/api/players?team=${teamId}`),
    getPerformance: (playerId, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.players?.getPerformance?.(playerId) || apiClient.get(`/api/players/${playerId}/performance`)
        : apiClient.get(`/api/players/${playerId}/performance`),
    getByTournament: (tournamentId, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.players?.getAll?.() || apiClient.get(`/api/players?tournament_id=${tournamentId}`)
  ...
  teams: {
    ...
    getFixtures: (teamId, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.teams?.getFixtures?.(teamId) || apiClient.get(`/api/teams/${teamId}/fixtures`)
  ...
  fantasyTeams: {
    getAll: (isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.fantasyTeams?.getAll?.() || apiClient.get("/api/fantasy-teams")
        : apiClient.get("/api/fantasy-teams"),
    getById: (id, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.fantasyTeams?.getById?.(id) || apiClient.get(`/api/fantasy-teams/${id}`)
        : apiClient.get(`/api/fantasy-teams/${id}`),
    create: (teamData, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.fantasyTeams?.create?.(teamData) || apiClient.post("/api/fantasy-teams", teamData)
        : apiClient.post("/api/fantasy-teams", teamData),
    update: (id, teamData, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.fantasyTeams?.update?.(id, teamData) || apiClient.put(`/api/fantasy-teams/${id}`, teamData)
        : apiClient.put(`/api/fantasy-teams/${id}`, teamData),
    delete: (id, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.fantasyTeams?.delete?.(id) || apiClient.delete(`/api/fantasy-teams/${id}`)
        : apiClient.delete(`/api/fantasy-teams/${id}`),
    optimize: (id, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.fantasyTeams?.optimize?.(id) || apiClient.post(`/api/fantasy-teams/${id}/optimize`)
  ...
  formations: {
    ...
    getById: (id, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.formations?.getById?.(id) || apiClient.get(`/api/formations/${id}`)
        : apiClient.get(`/api/formations/${id}`),
    getRecommended: (players, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.formations?.getRecommended?.(players) || apiClient.post("/api/formations/recommend", { players })
  ...
  analytics: {
    getPlayerStats: (playerId, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.analytics?.getPlayerStats?.(playerId) || apiClient.get(`/api/analytics/players/${playerId}`)
        : apiClient.get(`/api/analytics/players/${playerId}`),
    getTeamStats: (teamId, isSubscribed = true) => 
      shouldUseMockData(isSubscribed) 
        ? mockApi.analytics?.getTeamStats?.(teamId) || apiClient.get(`/api/analytics/teams/${teamId}`)
        : apiClient.get(`/api/analytics/teams/${teamId}`),
    getFormationStats: (formationId, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.analytics?.getFormationStats?.(formationId) || apiClient.get(`/api/analytics/formations/${formationId}`)
        : apiClient.get(`/api/analytics/formations/${formationId}`),
    getPredictions: (data, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.analytics?.getPredictions?.(data) || apiClient.post("/api/analytics/predictions", data)
        : apiClient.post("/api/analytics/predictions", data),
    getBestPlayers: (tournamentId, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.analytics?.getBestPlayers?.(tournamentId) || apiClient.get(`/best-players?tournament_id=${tournamentId}`)
        : apiClient.get(`/best-players?tournament_id=${tournamentId}`),
    getBestPlayersByTeams: (teams, tournamentId, isSubscribed = true) =>
      shouldUseMockData(isSubscribed) 
        ? mockApi.analytics?.getBestPlayersByTeams?.(teams, tournamentId) || apiClient.get(
